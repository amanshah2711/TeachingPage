...
class RetrainCallback(callbacks.BaseCallback):
    ...
    def __init__(self, total_timesteps, lr, embed_type, log_output_formats, embed_path, rl_algo, embed_types, embed_paths, embed_index, out_dir, freq, cls, *args, **kwargs):
        ...
        self.num_retrain = 1
        self.freq = freq
        self.log_output_formats = log_output_formats
    ...
    def _on_step(self) -> bool:
        if self.n_calls % self.freq == 0:
            ...
            train_fn(env=env, total_timesteps=self.total_timesteps, retrain=False, out_dir=out_dir, logger=None ,log_callbacks=[],save_callbacks=save_callbacks, extra_info={}, checkpoint_interval=float('inf'))
            self.num_retrain += 1

    def build_minimal_env(self):
        log_callbacks, save_callbacks = [], []
        _, logger = setup_logger(self.out_dir, exp_name='adversary_' + str(self.num_retrain), output_formats=self.log_output_formats, retrain=True)
        self.logger = logger
...
def _stable(
    ...
    if retrain:
        ...
        retrain_callback = RetrainCallback(embed_type=extra_info['embed_type'], embed_types=extra_info['embed_types'], embed_path=extra_info['embed_path'], embed_paths=extra_info['embed_paths'], embed_index=1-embed_index, lr=extra_info['lr'], out_dir=out_dir, cls=cls, rl_algo=extra_info['rl_algo'], total_timesteps=total_timesteps, freq = extra_info['retrain_freq'], log_output_formats=extra_info['log_output_formats'])
...
def train(
    ...
    res = train_fn(
        ...
        extra_info={
            ...
            'log_output_formats': log_output_formats
        }
    )
    single_venv.close()
