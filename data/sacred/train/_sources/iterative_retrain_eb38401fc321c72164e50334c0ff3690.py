...
def more_retrain(options):
    if options == []:
        print('Warning there are no checkpoints at this point in your checkpoint directory')
    checkpoints = [item for item in options if 'retrain' not in item]
    retrains =[item for item in options if 'retrain' in item]
    if retrains == []:
        return random_opponent(checkpoints)
    else:
        even = np.random.choice(checkpoints, size=9*len(retrains))
        options = retrains + even
    return np.random.choice(options)
...
class RetrainCallback(callbacks.BaseCallback):
    ...
    def __init__(self, total_timesteps, lr, embed_type, embed_path, rl_algo, embed_types, embed_paths, embed_index, out_dir, freq, cls, old_dir, *args, **kwargs):
    ...
    def _on_step(self) -> bool:
        ...
        train_fn(env=env, total_timesteps=int((self.num_timesteps) * 0.03), retrain=False, out_dir=out_dir, logger=None,log_callbacks=[],save_callbacks=save_callbacks, extra_info={}, checkpoint_interval=float('inf'), self_play=False)
        env.close()
        item = self.model.get_env()._policy
        load_params(item, out_dir)
        #logger.configure(folder = os.path.join(self.old_dir, 'rl'), format_strs = ['tensorboard','stdout'])
        self.num_retrain += 1
...
def _stable(
    ...
    if self_play:
        swap_callback = SwapCallback(out_dir, opponent_sampler=more_retrain)
        callback_list.append(swap_callback)

    if retrain:
      retrain_callback = callbacks.EveryNTimesteps(n_steps=extra_info['retrain_freq'], callback=RetrainCallback(embed_type=extra_info['embed_type'], embed_types=extra_info['embed_types'], embed_path=extra_info['embed_path'], embed_paths=extra_info['embed_paths'], embed_index=embed_index, lr=extra_info['lr'], out_dir=out_dir, cls=cls, rl_algo=extra_info['rl_algo'], total_timesteps=total_timesteps, freq = extra_info['retrain_freq'], old_dir = extra_info['out_dir']))
...
def train_config():
    ...
    retrain=True
    retrain_freq = 250000 #change later
