...
class RetrainCallback(callbacks.BaseCallback):
    ...
    def _on_step(self) -> bool:
        if self.n_calls % self.freq == 0:
            ...
            train_fn(env=env, total_timesteps=int(self.n_calls * 0.03), retrain=False, out_dir=out_dir, logger=self.logger,log_callbacks=[],save_callbacks=save_callbacks, extra_info={}, checkpoint_interval=float('inf'))
            item = self.model.get_env()._policy
