...
class RetrainCallback(callbacks.BaseCallback):
    ...
    def __init__(self, total_timesteps, lr, embed_type, embed_path, rl_algo, embed_types, embed_paths, embed_index, out_dir, freq, cls, *args, **kwargs):
        ...
        self.num_retrain = 1
        self.freq = freq
        self.env = self.build_minimal_env()
    ...
    def _on_step(self) -> bool:
        if self.n_calls % self.freq == 0:
            ...
            train_fn(env=self.env, total_timesteps=self.total_timesteps, retrain=False, out_dir=out_dir, logger=None,log_callbacks=[],save_callbacks=[], extra_info={}, checkpoint_interval=float('inf'))
            self.num_retrain += 1
